...
const INFERENCE_RULES: Array<{ kind: RelationKind; pattern: RegExp }> = [
  ...
  { kind: 'origin', pattern: /(от котор\w+ происходит переход|происходит переход|переход к|переход от|предшеству\w*|возникает из|из котор\w+ вырас\w*|становится основой|основа для|подготавлива\w*|приводит к|ведёт к|ведет к)/i },
  { kind: 'crisis', pattern: /(кризис|предел\w*|утрат\w*|распад|разрушени\w*|исчерпани\w*|обнаружение границ)/i },
