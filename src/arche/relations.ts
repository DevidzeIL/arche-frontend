 ...
 *
 * Шаблоны построены на ОСНОВАХ слов, а не на готовых словоформах.
 * В русском у одного смысла десятки форм: «отличие от», «отличается»,
 * «в отличие», «отличавшийся». Перечислять их поштучно бессмысленно —
 * каждая ненайденная форма молча роняет связь в «связано с» и вычёркивает
 * её из причинных цепочек. Поэтому ищем корень: `отлич`.
 *
 * Порядок важен. «Кризис, на который отвечает романтизм» должен стать
 * response, а не crisis, поэтому более специфичные правила идут раньше.
 */
const INFERENCE_RULES: Array<{ kind: RelationKind; pattern: RegExp }> = [
  { kind: 'response', pattern: /(ответ|отвеча|отклик|реакци)/i },
  ...
  { kind: 'develops', pattern: /(ученик|учител|последовател|преемни|наследу)/i },
  {
    kind: 'origin',
    pattern:
      /(переход|предшеству|возника|выраст|вырос|происхожд|становится основой|основа для|подготав|подготов|расшат|открыва\w+ (?:путь|возможность)|делает возможн|дал\w* начало|приводит к|ведёт к|ведет к|держится|порожда|зарожда|бер[её]т начало)/i,
  },
  { kind: 'crisis', pattern: /(кризис|предел|утрат|распад|разрушен|исчерпан|надлом|скомпрометир|пошатн|обнаж|подрыв|рушит)/i },
  { kind: 'opposes', pattern: /(противостои|противопоставл|противовес|отлич|отход|отказ|критик|вместо|альтернатив|наперекор|расходит|опроверг|спорит|против)/i },
  { kind: 'develops', pattern: /(развит|развива|развивш|систематиз|продолж|углубл|разработ|уточн|дораб|доводит)/i },
  { kind: 'author', pattern: /(автор|ключевое произведение|написа|создател|сочин)/i },
  { kind: 'epoch', pattern: /(эпох|период)/i },
  { kind: 'influence', pattern: /(влия|повлия|воздейств|формиру|сказал\w+ на)/i },
  { kind: 'context', pattern: /(контекст)/i },
];
